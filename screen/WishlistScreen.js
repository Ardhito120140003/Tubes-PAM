	...
	<View style={{ marginBottom: 10, marginLeft: 10, flexDirection: "row" }}>
		...
		>
			...
			>
				...
				<Text style={{ marginRight: 14, color: "white", marginTop: 4 }}>
					{item.rating}
				</Text>
			</View>
		</ImageBackground>
		<View style={{ flexDirection: "column", marginLeft: 20 }}>
			<Text style={{ fontWeight: "700", marginBottom: 4, fontSize: 16 }}>
				{item.name}
			</Text>
			<View style={{ flexDirection: "row", justifyContent: "flex-start" }}>
				<View>
					...
					<Text
						style={{ fontWeight: "400", marginBottom: 4, color: "#7A7E86" }}
					>
						{item.type}
					</Text>
				</View>
	...
	fetchData() {
		axios
			.all([
				...
				axios.get(
					"https://backend-uas-pam-production.up.railway.app/api/wishlist/" +
						this.state.username
				),
			])
			.then((response) => {
